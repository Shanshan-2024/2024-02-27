...
if __name__ == '__main__':
    ...
    #下一步需要把我的数据包装成data这样，有两个属性，data.x和data.edge_index
    data = dataset[0]

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    data = data.to(device)

    encoder = Encoder(dataset.num_features, activation,
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        #data.x: 形状[num_nodes, num_node_feature]
        #data.edge_index: 形状[2, num_edges]  torch.long
